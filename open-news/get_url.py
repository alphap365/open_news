...
def get_urls_from_rss_feed(feed_url: str, limit: int = 10) -> List[Dict]:
    ...
    try:
        ...
        for entry in feed.entries[:limit]:
            ...
            # Try to get feed title as source
...
def get_urls_from_google_news_search(query: str, limit: int = 10) -> List[Dict]:
    ...
    Keys: title, url (decoded), source, published, description.
    """
    ...
    try:
        ...
        for entry in feed.entries[:limit]:
            ...
            if not real_url:
                logger.debug(f"Skipping undecodable URL for '{title}'")
                continue

            # Extract source from feed's source element or title suffix
